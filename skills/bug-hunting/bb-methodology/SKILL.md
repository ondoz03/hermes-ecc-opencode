...
| Zendesk subdomain (`support.TARGET.com`) | Check Help Center public API: `/api/v2/help_center/categories.json` |
...
### Personal-project technique for mature-platform IDOR testing

When a mature target (GitLab, Zendesk, Shopify) restricts dangerous mutations/endpoints to admin/maintainer roles, a low-privilege account alone cannot determine if auth is properly enforced. The workaround:

1. **Create your own project** on the target platform (e.g., a Laravel project on GitLab)
2. **You become admin/owner** of that project automatically
3. **Test the dangerous mutations** from your admin position — confirm they work
4. **Switch accounts** (or use a different user's token) and retry the same mutations against your project
5. If the mutation still succeeds from a non-admin account on your project → **IDOR/missing-auth confirmed**
6. If it's blocked → the auth check is per-object, not per-user — note it, move on

This approach works because platforms typically do per-object permission checks (is this user a member of THIS project?) rather than per-user role checks. The vulnerability is when they forget to check: a user calls `projectDelete(id=your_project)` with someone else's auth, and it works.

**Where to apply:** GraphQL mutations with dangerous names (`*Destroy`, `*Delete`, `*Transfer`, `*Disable`, `admin*`), admin API endpoints, CI variable mutations, member management endpoints.

**Pitfall:** Some platforms cache permissions aggressively — flush token/session between admin and non-admin tests, or use separate browser sessions.

**GitLab-specific nuance:** GitLab blocks `__type(name: "Mutation")` introspection but still allows `__schema { types }` — iterate the full types array, find the type named `"Mutation"`, and read its `fields`. GitLab also validates GraphQL input arguments BEFORE checking auth, so a missing-argument error (`ARG_ERROR`) does not mean the mutation is blocked — you must send valid IDs to distinguish auth-denied from arg-invalid. Test experimental mutations (newer than ~GitLab 17.0, marked `**Status**: Experiment`) first — they receive less security review than stable ones.
